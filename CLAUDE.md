...
docs修正・1〜2行の自明な修正を除く、すべての変更に適用する。

1. **種別を宣言する**
   「この変更は〇〇（バグ修正 / 機能拡張 / 新機能 / 仕様に影響しない変更）です」と明示する。

2. **種別に応じた手順を完了してから実装に入る**
   各種別の前提手順（docs更新・Issue起票など）を省略したまま Edit/Write を呼ばない。

3. **変更内容を提示してユーザーの承認を得る**
   「どのファイルの何を変えるか」を説明し、ユーザーが承認を返すまで Edit/Write を呼ばない。
