...
- [ ] `Select` — ドロップダウン
- [ ] `Modal` — 確認ダイアログ
- [ ] `ScoreStars` — 1〜5 の星評価入力/表示
- [ ] `EmptyState` — 一覧が空のときのプレースホルダー
- [ ] `Spinner` — ローディング表示
...
- [ ] 登録済み求人をカード形式で表示
- [ ] 検索（企業名・職種・技術スタック）
- [ ] フィルタ（媒体、ステータス、リモート可否）
- [ ] ステータス別件数バッジをヘッダに表示
- [ ] 「+ 求人を追加」ボタン
- [ ] 比較チェックボックス（最大5件）→ 比較ボタンに遷移

### 4-2. 求人登録・編集フォーム (`src/pages/JobForm.tsx`)
- [ ] 基本情報セクション（企業名・職種・媒体・URL）
- [ ] 条件セクション（年収・勤務地・リモート・雇用形態等）
- [ ] スキル・技術セクション（タグ入力形式）
- [ ] メモセクション（業務内容要約・気になる点）
- [ ] ステータス・日程セクション
- [ ] バリデーション（企業名・職種は必須）
- [ ] 保存 / キャンセルボタン
...
- [ ] 評価軸の一覧表示（名前・重み・説明）
- [ ] ドラッグ&ドロップで並び替え（`@dnd-kit/core` 等）
- [ ] インライン編集（名前・重み・説明）
- [ ] 追加 / 削除
- [ ] デフォルト評価軸のプリセット（リモート・年収・技術・裁量・カルチャー・成長環境）

---

## Phase 6 — 評価入力機能

### 6-1. 評価入力UI (`src/pages/ScoreInput.tsx` または詳細画面内)
- [ ] 求人詳細内にタブ or セクションとして配置
- [ ] 各評価軸に対してスコア（1〜5）と根拠メモを入力
- [ ] 参照リンク（任意）の入力
- [ ] 入力済み軸の進捗バー表示（例：3/7軸完了）
- [ ] 一括保存
