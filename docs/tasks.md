...
- [x] 固定左カラム（軸名・重み）+ 横スクロール列（求人ごと、最低240px幅）
- [x] 列ヘッダカードに企業名・職種・StatusBadge・年収・リモート・勤務地・合致度%を表示
- [x] スコア対象軸 / 参考記録軸（weight=0）をセクション分けして行表示
- [x] 未評価セルはグレーの「—」表示（0点と区別）
- [x] 合致度スコア最大列をTOPバッジ＋青ハイライト
- [x] 列ヘッダの「評価を編集」から /jobs/:id/score へ遷移
- [x] 列ヘッダの × ボタンで比較から除外
- [x] 2件未満アクセス時は EmptyState を表示（リダイレクトなし）

---

## Phase 8 — ステータス管理

- [x] 求人一覧でステータス別タブ切り替え（Phase 4 で実装済み）
- [x] ステータス別件数バッジ（CountBadge）
- [x] ステータス更新は即時反映（確認ダイアログなし）

---

## Phase 9 — 設定画面 (`src/pages/Settings.tsx`)

- [x] 現在のデータ件数サマリー（求人・評価軸・評価）
- [x] JSONエクスポート（日付付きファイル名でダウンロード）
- [x] JSONインポート（バリデーション付き・成功/失敗フィードバック）
- [x] 全データリセット（ConfirmModal + initDefaults で評価軸を再投入）
- [x] スコア計算方式の表示（重み付き合計・読み取り専用）

---

## Phase 10 — 仕上げ・品質

- [x] レスポンシブ対応（カードグリッドは `grid-cols-1 sm:grid-cols-2 xl:grid-cols-3`）
- [x] 空状態 UI（JobList・Compare・CriteriaSettings に EmptyState）
- [x] キーボードショートカット（`/` キーで検索フォーカス）
- [x] `document.title` 動的設定（`useDocumentTitle` フック、全ページ対応）
- [x] MVP 受け入れ条件
  - 5件以上の求人登録・3軸以上での採点 → JobForm + ScoreInput で対応
  - 3件以上を比較画面で並べて差分確認 → Compare で最大5件・横スクロール
  - ステータス更新 → タブフィルタが即時反映
  - 合致度スコア計算・根拠メモが追える → JobDetail・Compare で表示
