...
ドメイン用語集・アーキテクチャ概要は [`docs/context.md`](docs/context.md) を参照してください。

---

<!-- 以下は旧内容。docs/context.md に統合済み -->
# 転職比較アプリ — ドメイン用語集
...
列順：勤務地 → 年収 → 始業時間 → リモート可否 → 年間賞与 → 年末年始休暇 → 住宅補助 → 雇用形態 → 必須スキル → 技術スタック → 応募ステータス → 媒体。
スキル系フィールドは先頭3件＋「+N」省略で表示する。
