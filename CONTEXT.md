...
## 比較画面 (Compare View)
複数の求人を評価軸ごとに並べるマトリクス表示。未評価のセルはグレーの「—」で表示する（0点表示との区別を視覚化）。
合致度スコアが数値最大の列（デスクトップ）または行（モバイル）を青くハイライトする（未入力の多寡は考慮しない）。
2件未満で /compare にアクセスした場合は空状態を表示し、求人一覧へ誘導する（リダイレクトはしない）。

**デスクトップ（md: 以上）**: 行=評価軸・列=求人。列ヘッダカードに年収・リモート・勤務地・ステータスを表示し、テーブル本体は評価軸のみとする。列幅は最低240px固定・横スクロール対応（最大5列でも視認性を維持）。評価軸の行順は CriteriaSettings の並び順に従う。

**モバイル（md: 未満）**: 行=求人・列=評価軸に転置する。左列（求人名・合致度）を固定し、評価軸列を横スクロールで確認する。列幅は均等割り付けで横スクロール対応。
